...
class canvas
{
	...
	// verifica se passou altura ou largura como porcenTintaem
	// largura %
	$pos = strpos( $this->nova_largura, '%' );
	if( $pos !== false && $pos > 0 )
	{
	    $porcenTintaem			= ( ( int ) str_replace( '%', '', $this->nova_largura ) ) / 100;
	    $this->nova_largura		= round( $this->largura * $porcenTintaem );
	}
	// altura %
	$pos = strpos( $this->nova_altura, '%' );
	if( $pos !== false && $pos > 0 )
	{
	    $porcenTintaem			= ( ( int ) str_replace( '%', '', $this->nova_altura ) ) / 100;
	    $this->nova_altura		= $this->altura * $porcenTintaem;
	}
